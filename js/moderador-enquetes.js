// =====================================================
// MODERADOR - MÓDULO DE ENQUETES (com Editar/Excluir)
// =====================================================
...
const ModuloEnquetes = (() => {
  ...
  function renderizarLista() {
    ...
    if (!enquetes.length) {
      container.innerHTML =
        '<p class="text-gray-500 text-center py-8">Nenhuma enquete criada</p>';
      return;
    }

    container.innerHTML = enquetes
      .map((e) => {
        ...
        <div class="border rounded-lg p-4 ${
          isAtiva ? 'bg-green-50 border-green-500 border-2' : 'bg-white'
        } hover:shadow-md transition">
          <div class="flex items-start justify-between mb-3">
            <div class="flex-1">
              <h4 class="font-semibold text-sm mb-1">${
                window.ModeradorCore.esc(e.titulo)
              }</h4>
              <p class="text-xs text-gray-600">
                ...
                ${
                  encerrada
                    ? ' • <span class="text-red-600">Encerrada</span>'
                    : ''
                }
              </p>
            </div>
            ${
              isAtiva
                ? '<span class="ml-2 text-xs bg-green-500 text-white px-3 py-1 rounded-full font-bold">ATIVA</span>'
                : ''
            }
          </div>
          ...
          <div class="flex flex-wrap gap-2">
            ...
            <!-- NOVOS BOTÕES -->
            <button 
              onclick="window.ModuloEnquetes.abrirModalEnqueteEditar('${e.id}')" 
              class="bg-blue-500 text-white text-xs px-3 py-2 rounded hover:opacity-90"
              title="Editar">
              ✏️
            </button>
            <button 
              onclick="window.ModuloEnquetes.excluirEnquete('${e.id}')" 
              class="bg-red-500 text-white text-xs px-3 py-2 rounded hover:opacity-90"
              title="Excluir">
              🗑️
            </button>
          </div>
        ...
        ${opcoes
          ...
          <div class="bg-white p-3 rounded-lg border">
            <div class="flex items-center justify-between mb-1">
              <span class="font-medium text-sm">${window.ModeradorCore.esc(
                opcao
              )}</span>
            ...
            <p class="text-xs text-gray-500 mt-1">${data[idx]} voto${
              data[idx] !== 1 ? 's' : ''
            }</p>
          </div>
        `
  ...
  function limparResultados() {
    const container = document.getElementById('resultadosEnquete');
    container.innerHTML =
      '<p class="text-gray-500 text-center py-8">Nenhuma enquete ativa</p>';
  ...
  async function encerrar(enqueteId) {
    if (
      !confirm('Encerrar esta enquete? Esta ação não pode ser desfeita.')
    ) {
      return;
    }
  ...
  function verDetalhes(enqueteId) {
    ...
    const opcoes = enquete.opcoes.opcoes
      .map((op, idx) => `${idx + 1}. ${op}`)
      .join('\n');
  ...
  function abrirModalEnqueteEditar(enqueteId) {
    ...
    editandoEnqueteId = enqueteId;

    document.getElementById('modalEnquete').classList.remove('hidden');
    document.getElementById('modalEnquete').classList.add('flex');

    // Preenche campos
    document.getElementById('inputTituloEnquete').value = e.titulo || '';
    document.getElementById('inputOpcoesEnquete').value = (e.opcoes?.opcoes || []).join('\n');
  ...
  function fecharModal() {
    ...
    editandoEnqueteId = null; // sair do modo edição
  }
    ...
    try {
      if (editandoEnqueteId) {
        ...
        const { data, error } = await supabase
          ...
          .insert([
            {
              palestra_id: null, // enquetes globais
              titulo: titulo,
  ...
  async function excluirEnquete(enqueteId) {
    ...
    try {
      // Se estiver ativa, desativar no controle primeiro
      const controle = window.ModeradorCore.state.controle;
      if (controle?.enquete_ativa === enqueteId) {
        await desativar(); // atualiza controle.enquete_ativa = null
      }
  ...
  async function exportarCSV() {
    ...
    try {
      ...
      link.download = `enquete_${enqueteAtiva.titulo.replace(
        /[^a-z0-9]/gi,
        '_'
      )}_${Date.now()}.csv`;
      link.click();
